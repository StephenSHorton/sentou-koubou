...
namespace Brennen.BrennenCode.Extensions;

//Mostly utilities to get asset paths.
public static class StringExtensions
{
    /// <summary>Godot resource paths need forward slashes; Path.Join uses backslash on Windows.</summary>
    ...
    {
        path = Res(MainFile.ResPath, "images", "card_portraits", "big", path);
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find big card image path: " + path);
        return Res(MainFile.ResPath, "images", "card_portraits", "big", "card.png");
    }
